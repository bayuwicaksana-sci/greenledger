        ...
        return [
            ...
            'classification' => [
                ...
                Rule::in([
                    ...
                    Program::CLASSIFICATION_NON_PROGRAM,
                ]),
            ],
            'program_type' => [
                'nullable',
                'string',
                Rule::in(['SINGLE_YEAR', 'MULTI_YEAR']),
            ],
            ...
            'estimated_harvest_date' => [
                'nullable',
                'date',
                'required_if:harvest_type,single',
            ],
            'harvest_frequency_value' => [
                'nullable',
                'integer',
                'min:1',
                'required_if:harvest_type,multiple',
            ],
            ...
            'trial_design_other' => [
                ...
                'string',
                'max:100',
                'required_if:trial_design,Other',
            ],
            ...
            'activities.*.planned_end_date' => [
                'nullable',
                'date',
                'after_or_equal:activities.*.planned_start_date',
            ],
            ...
            'budget_items.*.category_id' => [
                'required',
                'integer',
                'exists:program_budget_categories,id',
            ],
            'budget_items.*.phase_id' => [
                'required',
                'integer',
                'exists:program_budget_phases,id',
            ],
            'budget_items.*.item_description' => [
                'required',
                'string',
                'max:255',
            ],
            ...
            'completion_reason' => [
                'nullable',
                'string',
                'required_if:status,'.Program::STATUS_COMPLETED,
            ],
            'archive_reason' => [
                'nullable',
                'string',
                'required_if:status,'.Program::STATUS_ARCHIVED,
            ],
        ];
    }
        ...
        $validator->after(function (Validator $validator) {
            ...
            if (is_array($treatments)) {
                ...
                if (! empty($duplicates)) {
                    $validator
                        ...
                        ->add(
                            'treatments',
                            'Treatment codes must be unique within this program.',
                        );
                }
            }
            ...
            if ($fiscalYearId) {
                ...
                if ($fiscalYear && $fiscalYear->is_closed) {
                    ...
                    $user = auth()->user();
                    if (! $user || ! $user->can('program.override-closed-fy')) {
                        $validator
                            ->errors()
                            ->add(
                                'fiscal_year_id',
